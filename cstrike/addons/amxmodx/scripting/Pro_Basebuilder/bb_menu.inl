public globalMenu(id)
{
	...
	{
		menu_additem(menu, !g_boolCanBuild && g_boolRepick[id] ? "\r[ \wBuy Weapons\r ]" : "\r[ \dBuy Weapons\r ]");
	}
	
	menu_additem(menu, g_boolCanBuild ? "\r[ \dExtra Items\r ] \d[\rPrep Time\d]" : "\r[ \wExtra Items\r ]");
	...
	{
		menu_additem(menu, g_boolCanBuild ? "\r[ \wRespawn\r ]" : "\r[ \dRespawn\r ]");
	}
...
public PlayerMenu(id)
{
	...
	formatex(gText, charsmax(gText), "\r[ %sCurrent Color\r ]\d: \d[ \r%s\d ]", g_iColorMode != 2 ? "\w" : "\d", g_aColors[g_iColor[id]][Name]);
	menu_additem(menu, gText);
	
	formatex(gText, charsmax(gText), "\r[ \wBlock Mod\r ]\d: %s^n", !g_playerBlockRenderMode[id] ? "\wNormall" : "\dWithout color");
	menu_additem(menu, gText);
...
public helpingMenu_2(id, menu, item)
{
	...
	switch (adjustedItem)
	{
		...
		case 1:
		{
			...
			ManagementAction(id, target, "Opened the Help Menu for", "GodMode", "");
			UpdatePlayerGlow(target);
		}
		case 2:
		{
