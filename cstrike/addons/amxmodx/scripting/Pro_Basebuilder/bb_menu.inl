...
public PlayerMenu(id)
{
	...
	add_format_item(menu, "\r[ %sCurrent Color\r ]\d: \d[ \r%s\d ]", !g_isZombie[id] && g_boolCanBuild && g_iColorMode != 2 ? "\w" : "\d", g_aColors[g_iColor[id]][Name]);
	...
	if (access(id, FLAGS_VIP) && !g_isZombie[id] && g_boolCanBuild)
...
public SetRandomPlayerColor(id)
{
	...
	if (g_iColorMode)
	{
		...
		if (bCanSyncTeamColor)
		{
			...
			if (iOldTeammateColor != iOldColor && iOldTeammateColor >= 0 && iOldTeammateColor < MAX_COLORS)
			{
				g_iColorOwner[iOldTeammateColor] = 0;
			}
		}
		
		new availableColors[MAX_COLORS];
		new numAvailable = 0;
		for (new i = 0; i < MAX_COLORS - 1; i++)
		{
			if (!g_iColorOwner[i])
			{
				availableColors[numAvailable++] = i;
			}
		}
		...
		if (numAvailable == 0)
		{
			if (iOldColor >= 0 && iOldColor < MAX_COLORS)
			{
				g_iColorOwner[iOldColor] = id;
			}
			CC_SendMessage(id, "%s^x01 No other colors are currently available", MODNAME);
			return;
		}
		
		iColorIndex = availableColors[random_num(0, numAvailable - 1)];
