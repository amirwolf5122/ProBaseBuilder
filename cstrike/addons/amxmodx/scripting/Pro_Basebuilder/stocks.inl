...
ParseSettingValues(const szValue[], any:destin[], num_to_read, Float:min_clamp, Float:max_clamp, bool:is_float)
{
	new szTemp[3][16];
	parse(szValue, szTemp[0], charsmax(szTemp[]), szTemp[1], charsmax(szTemp[]), szTemp[2], charsmax(szTemp[]));
	
	for (new i = 0; i < num_to_read; i++)
	{
		if (is_float)
		{
			destin[i] = floatclamp(str_to_float(szTemp[i]), min_clamp, max_clamp);
		}
		else
		{
			destin[i] = clamp(str_to_num(szTemp[i]), floatround(min_clamp), floatround(max_clamp));
		}
	}
}
...
stock bool:BlockCheck(id, iEntity2, Float:ent2_mins[3], Float:ent2_maxs[3], BlockCheckType:checkType)
{
	...
	while ((iEntity1 = engfunc(EngFunc_FindEntityInSphere, iEntity1, center, radius)) != 0)
	{
		...
		get_block_bounds(iEntity1, ent1_mins, ent1_maxs);
...
stock get_block_bounds(ent, Float:out_mins[3], Float:out_maxs[3])
{
	...
	if (angles[0] == 0.0 && angles[1] == 0.0 && angles[2] == 0.0)
	{
		...
		xs_vec_add(origin, mins, out_mins);
		xs_vec_add(origin, maxs, out_maxs);
