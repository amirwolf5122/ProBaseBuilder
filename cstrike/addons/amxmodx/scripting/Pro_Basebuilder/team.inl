...
public menuTeamOption_2(id, menu, item)
{
	...
	switch (item)
	{
		...
		case 1:
		{
			...
			if (!ArePlayersOnSameTeam(id, teammate, true))
			{
...
public menuTeam(id)
{
	...
	{
		if (!ArePlayersOnSameTeam(id, i, true))
		{
			continue;
		}
...
public menuTeam_2(id, menu, item)
{
	...
	switch (item)
	{
		...
		default:
		{
			...
			if (!ArePlayersOnSameTeam(id, target, true))
			{
...
public menuConfirmationTeam_2(id, menu, item)
{
	...
	switch(item)
	{
		case 0:
		{
			...
			if (!ArePlayersOnSameTeam(id, target, false))
			{
...
public teamLineOrSprite(id)
{
	...
	if (!ArePlayersInSameParty(id, team_mate) || !is_user_alive(team_mate) || g_isZombie[team_mate])
	{
		return;
	}
...
stock bool:ArePlayersOnSameTeam(player1, player2, bool:check_alive = false)
{
	...
	if (check_alive && (!is_user_alive(player1) || !is_user_alive(player2)))
	{
		return false;
	}
...
stock bool:ArePlayersInSameParty(player1, player2)
{
	if (!is_user_connected(player1) || !is_user_connected(player2))
		return false;
...
{
	if (!ArePlayersInSameParty(player1, player2))
	{
		return false;
	}
	if (g_boolCanBuild)
	{
		if (!ArePlayersOnSameTeam(player1, player2, false))
		{
			return false;
		}
	}
...
public leaveParty(id)
{
	...
	userTeam[id] 		= 0;
	userTeam[target] 	= 0;
	CC_SendMessage(target, "%s ^x04[Team]^x01 Your team was disbanded", MODNAME);
}
