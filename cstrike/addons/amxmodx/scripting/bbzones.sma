...
public origin_check(id, ent)
{
	new Float:entCenter[3]
	new zoneEnt = -1
	new Float:zCenter[3]
	...
	while((zoneEnt = find_ent_by_class(zoneEnt, g_szClassname)))
	{
		...
		pev(ent, pev_absmin, flAbsMin)
		pev(ent, pev_absmax, flAbsMax)
		
		entCenter[0] = (flAbsMin[0] + flAbsMax[0]) * 0.5
		entCenter[1] = (flAbsMin[1] + flAbsMax[1]) * 0.5
		entCenter[2] = (flAbsMin[2] + flAbsMax[2]) * 0.5
		
		// Get world center for the zone
		static Float:zAbsMin[3], Float:zAbsMax[3]
		pev(zoneEnt, pev_absmin, zAbsMin)
		pev(zoneEnt, pev_absmax, zAbsMax)
		
		zCenter[0] = (zAbsMin[0] + zAbsMax[0]) * 0.5
		zCenter[1] = (zAbsMin[1] + zAbsMax[1]) * 0.5
		zCenter[2] = (zAbsMin[2] + zAbsMax[2]) * 0.5
		...
		pev(zoneEnt, pev_size, zoneSize)
		pev(ent, pev_size, entSize)
		
		new Float:zHalfSize[3]
		new Float:entHalfSize[3]
		
		zHalfSize[0] = zoneSize[0] / 2
		zHalfSize[1] = zoneSize[1] / 2
		zHalfSize[2] = zoneSize[2] / 2
		
		entHalfSize[0] = entSize[0] / 2
		entHalfSize[1] = entSize[1] / 2
		entHalfSize[2] = entSize[2] / 2
		
		new Float:maxDistX
		new Float:maxDistY
		new Float:maxDistZ
		
		maxDistX = zHalfSize[0] + entHalfSize[0]
		maxDistY = zHalfSize[1] + entHalfSize[1]
		maxDistZ = zHalfSize[2] + entHalfSize[2]
		
		if((zCenter[0] - entCenter[0] > maxDistX) || (zCenter[0] - entCenter[0] < -maxDistX) 
